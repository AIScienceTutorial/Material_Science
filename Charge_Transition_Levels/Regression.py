...
# Next, we will load the DFT data and descriptors. The data are based on DFT calculations of defect atoms (which can have different charges) in Cd-based compounds, CdX, where X is Te, Se, or S, and contain the defect charge transition level, which is the Fermi level (or electron chemical potential) at which a particular defect changes form one charge state to another. We will also read in descriptors that are based on (a) the elemental properties of the defect atom, and (b) properties computed from cheaper unit cell defect calculations. Further description of the DFT data and descriptors can be obtained from the attached presentation or the publication. We will train the model on different sets of descriptors and see what works best.
# 
# Below, the DFT data + set of descriptors is read from a Data.csv file. Every data point is an impurity atom + defect site + CdX compound + DFT computed set of properties + complete set of descriptors. The DFT property to be read here (in column 3) is the combined set of q1/q2 charge transition levels. 1st set of descriptors is from column 5 to column 20, 2nd set of descriptors is from column 21 to column 25, and the complete set of descriptors is when columns 5 to 25 are used. Two further .csv files are read: Outside.csv which contains additional DFT computations performed on new compounds (CdTeSe and CdSeS alloys) and X.csv which contains descriptors for thousands of possible data points that make up the chemical space, about 20% of which were used to generate the DFT dataset. 
# 
# Specifically, there are 1827 total impurities possible in the 5 compounds (CdTe, CdTeSe, CdSe, CdSeS, CdS), of which DFT computations of charge transition levels are performed for 381 impurities. Since 6 types of charge transition levels are studied here (for charges from +3 to -3), total possible data points = 1827×6 = 10962, which is the size of the X.csv dataset. DFT data points in Data.csv equal 381×6 = 2286 in total. Outside.csv contains calculations on 44 new impurities in CdTeSe and CdSeS, which lead to 44×6 = 264 data points that are used for "out-of-sample" validation.
# 
...
plt.title('RFR Model, Charge Transition Levels (eV)', fontsize=20, pad=12)
#plt.title('KRR Model, Charge Transition Levels (eV)', fontsize=20, pad=12)
...
plt.text(2.05, 0.0, 'Out_rmse = ', c='r', fontsize=16)
plt.text(3.3, 0.0, out, c='r', fontsize=16)
plt.text(3.8, 0.0, 'eV', c='r', fontsize=16)
plt.text(2.0, -0.5, 'Test_rmse = ', c='r', fontsize=16)
plt.text(3.3, -0.5, te, c='r', fontsize=16)
plt.text(3.8, -0.5, 'eV', c='r', fontsize=16)
plt.text(1.9, -1.0, 'Train_rmse = ', c='r', fontsize=16)
plt.text(3.3, -1.0, tr, c='r', fontsize=16)
plt.text(3.8, -1.0, 'eV', c='r', fontsize=16)
